...
sf::Sound* SoundEngine::loadSound(string path)
{
	...
	try{
		...
		}catch(exception& e)
		{
			...
			logger->log(strcat("SoundEngine::loadSound(string path) Excepción capturada SFML: ", e.what()));
			return NULL;
		}
	}	
}
...
void SoundEngine::playSound(Sound* sound, float volume, bool loop)
{
	...
	}catch(exception& e)
	{
		...
		logger->log(strcat("SoundEngine::playSound(...) Excepción capturada SFML: ", e.what()));
	}
...
bool SoundEngine::deleteSound(string path)
{
	...
	else
	{
		...
		}catch(exception& e)
		{
			...
			logger->log(strcat("SoundEngine::deleteSound(string path) Excepción capturada SFML: ", e.what()));
			return false;
		}
	}
}
...
void SoundEngine::stopSound(Sound* sound)
{
	if (isSoundPlaying(sound)) //Si el sonido está reproduciéndose, se detiene.
	{
		...
		}catch(exception& e)
		{
			...
			logger->log(strcat("SoundEngine::stopSound(Sound* sound) Excepción capturada SFML: ", e.what()));
		}
	}
}
...
sf::Music* SoundEngine::loadMusic(string path)
{
	...
	{ 
		...
		}catch(exception& e)
		{
			...
			logger->log(strcat("SoundEngine::loadMusic(string path) Excepción capturada SFML: ", e.what()));
		}
	}
}
...
{	
	...
	}catch(exception& e)
	{
		...
		logger->log(strcat("SoundEngine::playMusic(Music* music, float volume, bool loop, bool withStop) Excepción capturada SFML: ", e.what()));
	}
}
...
void SoundEngine::pauseMusic()
{
	...
	{
		logger->log(strcat("SoundEngine::pauseMusic() Excepción capturada SFML: ", e.what()));
	}
}
...
void SoundEngine::stopMusic()
{
	...
	{
		logger->log(strcat("SoundEngine::stopMusic() Excepción capturada SFML: ", e.what()));
	}
}
...
void SoundEngine::setLoop(bool loop)
{
	...
	{
		logger->log(strcat("SoundEngine::setLoop() Excepción capturada SFML: ", e.what()));
	}
}
