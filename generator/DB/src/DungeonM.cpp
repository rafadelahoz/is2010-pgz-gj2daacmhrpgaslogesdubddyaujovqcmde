...
Screen* DungeonM::make_door(Screen* s, int* connected, int* current_room, int dir_or, short area) {
    ...
    Screen* next_room = NULL;                // Siguiente habitación a visitar
    int dir_dest;                            // Dirección de la puerta de la habitación a visitar
    vector<Screen*>* aux = areas[area];      // Simplemente para acortar un poco el código, puntero al área en cuestión
...
void DungeonM::connect_rooms(short area) {
    ...
    Screen* s = areas[area]->at(0);     // Habitación actual (inicialmente, la primera del vector)

    int current_room = 0;               // Índice de la habitación actual dentro del vector de zona

    while (!finished) {                 // Mientras no haya terminado...
        Screen* s1 = make_door(s, connected, &current_room, rand() % 4, area); // Elegimos una dirección al azar e intentamos hacer una puerta ahí
        if (s1 != NULL)
